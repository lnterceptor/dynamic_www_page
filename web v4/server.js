...
const products = [
  ...
  products.forEach((product) => {
    db.run('INSERT INTO products (product, product_category) VALUES (?, ?)',
...
db.get("SELECT COUNT(*) AS count FROM products", (err, row) => {
  ...
  if (rowCount === 0) {
    db.run(`INSERT INTO activities(activity) VALUES ('baking'), ('cooking'), ('grilling'), ('roasting'), ('steaming'), ('boiling'), ('frying'), ('sauteing'), ('poaching'), ('smoking');`, (err) => {
     if (err) {
       return console.error(err.message);
     }
...
db.get("SELECT COUNT(*) AS count FROM categories", (err, row) => {
  ...
  if (rowCount === 0) {
    db.run(`INSERT INTO categories(category) VALUES ('Dairy'), ('Meat'), ('Fish'), ('Dessert');`, (err) => {
     if (err) {
       return console.error(err.message);
     }
     console.log('Records inserted successfully.');
...
app.post("/register", async(req, res) =>{4
    let db = new sqlite3.Database('data.db')

    db.run(`INSERT INTO users (username, password, email) VALUES (?, ?, ?)`, [req.body.username, req.body.password, req.body.email], (err) => {
        if (err) {
            console.log(err)
...
app.post("/addToDataBase", async(req, res) =>{
  ...
  if(req.body.addCategory !== ""){
    ...
    db.run(`INSERT INTO categories (category) VALUES (?)`, [req.body.addCategory], (err) => {
      db.close()
    });
...
app.post("/addToDataBase2", async(req, res) =>{
  ...
  if(req.body.addProduct !== ""){
    ...
    let db = new sqlite3.Database('data.db')
      ...
      db.run(`INSERT INTO products (product, product_category) VALUES (?, ?)`, [req.body.addProduct, req.body.selectCategory], (err) => {
      db.close()
      });
  }
  res.redirect("/admin/addToDataBase")
...
app.post('/addDishList', (req, res) => {
  ...
  if(id == -1){
    ...
    db.run(`INSERT INTO recipes (user, przepis_json) VALUES (?, ?)`, [username, jsonDataString], (err) => {
    db.close()
  });
...
app.get('/admin/addToDataBase',(req, res) =>{
  ...
  const username = req.query.username;
    ...
    db.all('SELECT category FROM categories', (err, result) => {
      ...
      res.render("addToDataBase.ejs", {username:username, category: JSON.stringify(categoriesData), products:JSON.stringify(productsData)})
    })
  })
});
...
app.get('/homepage',(req, res) =>{
    ...
    db.all('SELECT user, przepis_json, id FROM recipes', [], (err, result) => {
      ...
      res.render("homepage.ejs", {username:username, recipes: JSON.stringify(recipesData)})
    })
...
app.get('/AddDishList',(req, res) =>{
    ...
    db.all('SELECT category FROM categories', (err, result) => {
          ...
          const categoriesData = result.map(row => ({
            ...
            res.render("AddDishList.ejs", {username, id:newId, editJson:editableJSON, activities: JSON.stringify(activitiesData), products: JSON.stringify(productsData), categories: JSON.stringify(categoriesData)});
        });
    });
  });
})
...
app.post("/deleteProduct", async(req, res) =>{
  ...
  if(product != "" && category != ""){
    let db = new sqlite3.Database('data.db');
    db.run(`DELETE FROM products where product_category = (?) AND product = (?)`, [category, product], (err) => {
       db.close()
     });
     res.redirect("/admin/addToDataBase");
  }
